...
class Orders_Biddings(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if orders:
           print("there are orders today")
        if biddings:
           print("current bid _____ 2")

        for bid in biddings:
          print("Hello")
          try: 
             this_bid = BiddingsModel.objects.get(driver_email__exact=request.user.email)
             if this_bid:
             ...
             logging.error("Delivery driver does not exist")
